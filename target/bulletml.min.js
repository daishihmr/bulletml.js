...
a.bullet.getWalker());var d={x:this.x+a.option.offsetX,y:this.y+a.option.offsetY},f=a.direction||a.bullet.direction,k=f.value*Math.PI/180;switch(f.type){case "aim":c.direction=this.config.target?a.option.autonomy?b(d,this.config.target)+k:b(this,this.config.target)+k:k-Math.PI/2;break;case "absolute":c.direction=k-Math.PI/2;break;case "relative":c.direction=this.direction+k;break;default:c.direction=this.lastDirection+k}this.lastDirection=c.direction;f=a.speed||a.bullet.speed;k=f.value;switch(f.type){case "relative":c.speed=
this.speed+k;break;case "sequence":c.speed=this.lastSpeed+k;break;default:c.speed=k}this.lastSpeed=c.speed;c.x=d.x;c.y=d.y;var d={},q;for(q in a.bullet.option)d[q]=a.bullet.option[q];d.label=a.bullet.label;this.config.createNewBullet(c,d)};bulletml.runner.SubRunner.prototype.changeDirection=function(a){var e=a.direction.value*Math.PI/180,d=a.term;switch(a.direction.type){case "aim":this.dirFin=b(this,this.config.target)+e;this.dirIncr=c(this.dirFin-this.direction)/d;break;case "absolute":this.dirFin=
